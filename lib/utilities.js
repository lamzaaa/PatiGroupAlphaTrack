...
export async function appendScripts(scripts) {
	const loadPromises = scripts.map(scriptUrl => {
		const isExitScript = document.querySelector(`script[src="${scriptUrl}"]`);
...
export async function appendStyles(styles) {
	const loadPromises = styles.map(styleUrl => {
		...
		return new Promise((resolve, reject) => {
			...
			style.onerror = () => reject(new Error(`Failed to load ${styleUrl}`));
		});
	});
	...
	splitters.each(function () {
		...
		if (configString.includes(':')) {
			...
			const activeBreakpoint = sortedBreakpoints.find(
				key => window.matchMedia(`(min-width: ${BREAKPOINTS[key]}px)`).matches
			);
...
export function countLines(target) {
	...
	if (box_sizing == 'border-box') {
		...
		const contentsSpacing = allContent.toArray().reduce((acc, item) => {
			...
			const margin_bottom = parseInt(style.getPropertyValue('margin-bottom'));
	...
	$('[separate-group]').each(function () {
		...
		items.each(function (index) {
			...
			if (!isLastItemInRow && !isLastItemOverall) {
				const $separator = $(`<span separate-line="">${content}</span>`);
		...
		$this.find('[separate-line]').each(function () {
			...
			if ($prevItem.length > 0) {
				...
				if (maxHeightPerRow[rowOfPrevItem]) {
					$(this).css('--data-height', maxHeightPerRow[rowOfPrevItem] + 'px');
				}
			}
		});
...
export function autoAlignElements(selector = '[auto-align]') {
	$(selector).each(function () {
		...
		items.each(function () {
			...
			if (currentRowTop === null || Math.abs(itemTop - currentRowTop) < 5) {
...
export function customTab() {
	$('[tab-wrapper]').each(function () {
		...
		let activeValue =
			$this.find(`[tab-item=${tabValue}].active`).attr('tab-item-value') ||
			$this.find(`[tab-item=${tabValue}]`).first().attr('tab-item-value');

		let hashTag = window.location.hash.substring(1);
		if (
			hashTag &&
			$this.find(`[tab-item=${tabValue}][tab-item-value='${hashTag}']`).length >
				0
		) {
			activeValue = hashTag;
		}
		...
		const activateTab = value => {
			...
			$this.find(`[tab-content=${tabValue}]`).removeClass('active').hide();
			...
			if (!hasActiveTab) {
				...
				if ($defaultTab.length > 0) {
					...
					$this
						.find(`[tab-content=${tabValue}][tab-content-value='-1']`)
						.addClass('active')
						.fadeIn(300);
				}
			}
...
export function toggleFaqsAskModule() {
	$('.module-toggle').each(function () {
		...
		if (split > 1) {
			...
			for (let i = 0; i < split; i++) {
				const $column = $(`<div class="split-column split-column-${i}"></div>`);
				$this.append($column);
			}
		...
		function handleClickSingleColumn(currentItem) {
			...
			if (currentItem.hasClass('active')) {
				if (openAtleastOne === 'false') {
					...
					$this.find('.item').eq(index).find('.top').removeClass('active');
		...
		function handleClickMultiColumn(currentItem) {
			...
			const parentColumnIndex = currentItem.closest('.split-column').index();
